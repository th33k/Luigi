...
cap.set(3, 480)  # Set the width of the video capture
cap.set(4, 320)  # Set the height of the video capture
timer = 0
stateResult = False
startGame = False
score = [0, 0]
...
def gameScores():
    ...
    if gameScoreProgressBarCounter >= 80:  # If the game score progress bar is complete
        gameScoreProgressBarCounter = 0
...
def main():
    ...
    while True:
        ...
        if videoReadingStatus:
            ...
            # Check if we need to exit after FINAL_SCORES
            if currentState == 'HOME':
                break  # Exit the loop after final scores
